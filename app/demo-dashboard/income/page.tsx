...
const SAMPLE_INCOME: IncomeSource[] = [
  { id: "1", source: "Salary - Tech Corp", amount: 3200, frequency: "biweekly", category: "Salary" },
  { id: "2", source: "Freelance - Web Design", amount: 1500, frequency: "monthly", category: "Freelance" },
];

function IncomePageContent() {
  const { isPinned } = useSidebar();
  const [income, setIncome] = useState<IncomeSource[]>(SAMPLE_INCOME);
  ...
  const saveInlineEdit = (sourceId: string, paymentIndex: number) => {
  ...
  const handleEditIncome = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Add individual amounts for weekly/biweekly
    if (frequency === "weekly" || frequency === "biweekly") {
      updatedIncome.amounts = editPaymentAmounts.length > 0 ? editPaymentAmounts : undefined;
    } else {
      // Remove amounts if frequency changed to monthly
      updatedIncome.amounts = undefined;
    }

    setIncome(income.map(inc => inc.id === editingIncome.id ? updatedIncome : inc));
  ...
  const confirmDelete = () => {
    if (deletingIncomeId) {
      setIncome(income.filter(inc => inc.id !== deletingIncomeId));
          ...
          <div className="modal-box w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-base-content/60 mt-1">Update income information</p>
              </div>
              <button
                onClick={() => {
                  setShowEditModal(false);
            ...
            <form onSubmit={handleEditIncome} className="space-y-4">
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="form-control">
                    <label className="label">
                      <span className="label-text font-medium">Amount per Payment *</span>
                    </label>
                    <input
                      type="number"
                      name="amount"
                  ...
                  <div className="form-control">
                    ...
                    <select name="frequency" defaultValue={editingIncome.frequency} className="select select-bordered w-full" required>
                      <option value="weekly">Weekly (4 payments/month)</option>
                      <option value="biweekly">Bi-weekly (2 payments/month)</option>
                      <option value="monthly">Monthly (1 payment/month)</option>
                    </select>
                ...
                {/* Individual Payment Amount Inputs for Weekly */}
                {editingIncome.frequency === "weekly" && editPaymentAmounts.length > 0 && (
                  <div className="mt-4 space-y-3">
                    <div className="text-sm font-medium text-base-content/70">
                      Individual Weekly Payments
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {editPaymentAmounts.map((amount, index) => (
                        <div key={index} className="form-control">
                          <label className="label">
                            <span className="label-text text-sm">Week {index + 1} Amount</span>
                          </label>
                          <input
                            type="number"
                            step="0.01"
                ...
                {/* Individual Payment Amount Inputs for Biweekly */}
                {editingIncome.frequency === "biweekly" && editPaymentAmounts.length > 0 && (
                  <div className="mt-4 space-y-3">
                    <div className="text-sm font-medium text-base-content/70">
                      Individual Bi-weekly Payments
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {editPaymentAmounts.map((amount, index) => (
                        <div key={index} className="form-control">
                          <label className="label">
                            <span className="label-text text-sm">
                              {index === 0 ? "1st Paycheck" : "2nd Paycheck"} Amount
                            </span>
                          </label>
                          <input
                            type="number"
